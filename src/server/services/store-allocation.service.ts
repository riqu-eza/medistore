...
export async function validateStoreForDrug(
  ...
  if (drug.storageConditionGroup) {
    ...
    if (drug.storageConditionGroup === 'controlled' && storeType !== 'controlled') {
...
export async function suggestStoreForDrug(
  ...
  const scored = await Promise.all(
    stores.map(async (store) => {
      ...
      // Temperature match
      const storageReqs = drug.storageRequirements as any
      if (storageReqs?.temp_min !== undefined && storageReqs?.temp_max !== undefined) {
        if (store.temperatureMin && store.temperatureMax) {
          ...
          if (storageReqs.temp_min >= storeMin && storageReqs.temp_max <= storeMax) {
            score += 30
...
export async function allocateBatchToStore(
  ...
  if (!batch.currentStoreId.isReceivingZone) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const updatedReceivingQty = receivingInventory.availableQuantity.toNumber() - input.quantity
    ...
    if (targetInventory) {
      ...
      const newQty = targetInventory.availableQuantity.toNumber() + input.quantity
      await tx.inventory.update({
      ...
      await tx.inventoryLedger.create({
        data: {
          ...
          referenceType: 'TRANSFER',
          referenceId: transfer.id,
          notes: `Transfer from ${batch.currentStoreId!.name}`
        }
      })
    } else {
      ...
      await tx.inventory.create({
        data: {
          ...
          isExpired: false,
          isNearExpiry: batch.expiryDate < new Date(Date.now() + 90 * 24 * 60 * 60 * 1000),
          isLowStock: false,
          lastMovementDate: new Date(),
          lastMovementType: 'TRANSFER_IN'
        }
...
export async function bulkAllocateFromGRN(
  ...
  const validations = await Promise.all(
    input.allocations.map(async (alloc) => {
      ...
      if (!batch) {
        return { valid: false, error: `Batch ${alloc.batchId} not found in GRN` }
      }
 ...
 */
export async function autoAllocateBatches(
  grnId: string,
  session: Session
) {
  // Permission check
  ...
  for (const batch of grn.batches) {
    ...
    if (receivingInventory && receivingInventory.availableQuantity.toNumber() > 0) {
      allocations.push({
        batchId: batch.id,
