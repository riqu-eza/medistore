...
export async function POST(
  ...
  try {
    ...
    if (
      ...
      temperature < store.temperatureMin.toNumber()
    ) {
      isAlert = true;
      alertReason = `Temperature ${temperature}°C is below minimum threshold ${store.temperatureMin}°C`;
    } else if (
      store?.temperatureMax &&
      temperature > store.temperatureMax.toNumber()
    ) {
      isAlert = true;
    ...
    if (humidity !== null) {
      if (store?.humidityMin && humidity < store.humidityMin.toNumber()) {
        isAlert = true;
        ...
        store?.humidityMax &&
        humidity > store.humidityMax.toNumber()
      ) {
        isAlert = true;
        alertReason = alertReason
