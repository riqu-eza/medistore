...
export default function DashboardLayout({
  ...
  const navigationItems = getNavigationItems(user.roleName?.toLowerCase() || '')
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      {/* Desktop Sidebar */}
      <aside
        className={`hidden lg:flex flex-col bg-gradient-to-b from-slate-900 to-slate-800 text-white transition-all duration-300 ${
          ...
          {visibleNavItems.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={`${isActive ? 'text-blue-400' : 'text-slate-400 group-hover:text-white'} transition-colors`}>
                  {item.icon}
                </div>
                {isSidebarOpen && (
                  <>
          ...
          <div className={`flex items-center gap-3 ${!isSidebarOpen && 'justify-center'}`}>
            ...
            {isSidebarOpen && (
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 truncate capitalize">{user.roleName}</p>
              </div>
            )}
          </div>
        </div>
          ...
          <aside className="absolute inset-y-0 left-0 w-72 bg-gradient-to-b from-slate-900 to-slate-800 flex flex-col shadow-2xl">
            ...
            <div className="h-16 flex items-center justify-between px-5 border-b border-slate-700/50">
              <div className="flex items-center gap-3">
                ...
                <span className="text-lg font-bold text-white">PharmaTrace</span>
              </div>
              <button
                onClick={() => setIsMobileMenuOpen(false)}
              ...
              {showUserMenu && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-200 py-2 z-50">
                  <div className="px-4 py-3 border-b border-slate-100">
                    ...
                    <p className="text-xs text-slate-400 mt-1 capitalize">{user.roleName}</p>
                  </div>
                  <Link
                    href="/settings"
