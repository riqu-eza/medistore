...
import AllocationWizard from "../allocations/allocation-wizard";
import { useGRN, useGRNs } from "@/hooks/use-grn";
...
export default function ReceivingWorkflow({
  ...
  const { grn, loading, refetch } = useGRN(grnId);
  console.log(grn)
  const [activeTab, setActiveTab] = useState("grn");
  const status = grn?.status;
  const handleGRNApproved = async () => {
    await refetch(); // 🔥 pull real DB state
  };
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center justify-center gap-4">
            ...
            {/* Arrow */}
            <div
              className={`w-12 h-0.5 ${status === "approved" ? "bg-green-500" : "bg-muted"}`}
            />
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="  grid w-full grid-cols-2">
          ...
          >
            ...
            Store Allocation
            {!isApproved && (
              <Badge variant="secondary" className="text-red-600 ml-2">
                Locked
              </Badge>
            )}
          </TabsTrigger>
        ...
        <TabsContent value="grn" className="mt-6">
          <GRNDetailView grnId={grnId} onApproved={handleGRNApproved} />
        </TabsContent>
